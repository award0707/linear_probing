#ifndef ONE_RB_QUERYTESTER_HPP
#define ONE_RB_QUERYTESTER_HPP
...
class one_rb_querytester {
	...
	public:
	one_rb_querytester(pcg64 &r, std::vector<int> const &x,
	            std::vector<uint64_t> const &b, int nq, int nt, int fp,
	            int mx)
	           : rng(r), xs(x), bs(b), nqueries(nq), ntests(nt),
	             fail_pct(fp), magic_x(mx) {
		run_test();
	}

	friend
	std::ostream& operator<<(std::ostream& os, one_rb_querytester const& h) {
